...
class PushSendToCRM(generics.ListCreateAPIView):
    ...
    def updateCRM(self, serialized_data: dict, project_data: dict):
        ...
        # LOOP HERE
        techFinalizedLaborHours = 0
        engineeringFinalizedLaborHours = 0
        items = project_data.get("Items")
        for each_item in items:
            laborType = each_item.get("LaborTypes")
            match laborType:
                ...
                case "Engineering Labor":
                    TotalLaborHours = each_item.get("TotalLaborHours") or 0
                    engineeringFinalizedLaborHours += TotalLaborHours
                    # Engineering Labor
                case _:
                    print(f"Unkown labor type: {laborType}")
            print(each_item)
            ...
            #if isinstance(TotalLaborHours, (int, float)) and TotalLaborHours >= 0:
            ...
            if TotalLaborHours is None:
                logger.warning("Missing TotalLaborHours: %s", each_item)
                continue
